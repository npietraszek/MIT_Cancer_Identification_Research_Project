...
def get_the_rotated_files():
    ...
    number_testing_to_save_cancer_cells = 200
    number_testing_to_save_fibroblasts = 200
    ...
    for the_dir in total_directory_list:
        ...
        if the_label_matrix[0] == 0:
            ...
            if testing_saving_cancer_counter <= number_testing_to_save_cancer_cells:
                for x in range(1, 5):
            ...
            if testing_saving_fibroblast_counter <= number_testing_to_save_fibroblasts:
                for x in range(1, 5):
                    saved_testing_data_dict["saved_testing_data_{0}".format(x)].append(
                        os.path.join(the_dir, "Final_5D_array_{0}".format(x) + ".npy"))
                    # print(validation_data_dict["validation_data_{0}".format(x)][0])
...
saved_superloop_maximum_accuracy = 0
saved_superloop_testing_loss_with_maximum_accuracy = 0
saved_superloop_epoch_with_maximum_accuracy = 0
saved_superloop_counter_maximum_accuracy = 0

saved_superloop_minimum_testing_loss = 100000
saved_superloop_accuracy_with_minimum_testing_loss = 0
saved_superloop_epoch_with_minimum_testing_loss = 0
saved_superloop_counter_minimum_testing_loss = 0
...
for superloop_counter in range(superloop_rounds):
    ...
    weight_save_folder=r"D:\MIT_Tumor_Identifcation_Project_Stuff\Phase_2_stuff\Machine Learning\Weights\1_9_21_new_run_2_(new_model_4,no_standard-harsh_bal(20-50-50),1600_seperate_saving,rotate)\Superloop_run_{0}".format(superloop_counter)

    shortened_weight_save_folder = r"D:\MIT_Tumor_Identifcation_Project_Stuff\Phase_2_stuff\Machine Learning\Weights\1_9_21_new_run_1_(new_model_4,no_standard-harsh_bal(20-50-50),saving,rotate)"
    ...
    filepath=os.path.join(weight_save_folder,"1_9_21_Test2_balanced_0.001learning_DNN.{epoch:02d}.hdf5")
    filename=os.path.join(weight_save_folder,'training.log')
    checkDirectory(weight_save_folder)

    epoch_name_part_1 = r"1_9_21_Test2_balanced_0.001learning_DNN."
    ...
    saved_test_gen = nicholas_generator(final_saved_testing_data,final_saved_testing_labels,batch_sz)
    ...
    for epoch_counter in range(nb_epochs):
        ...
        print("Cells that were wrong: " + str(cells_that_were_wrong_list))
        cells_that_were_wrong_list = []
        wrong_fibroblasts = 0
        wrong_cancer_cells = 0
        is_correct = False
    ...
    # Now to test the 1600 saved cells on the model that had the highest accuracy.

    saved_maximum_accuracy = 0
    saved_testing_loss_with_maximum_accuracy = 0
    saved_epoch_with_maximum_accuracy = 0

    saved_minimum_testing_loss = 100000
    saved_accuracy_with_minimum_testing_loss = 0
    saved_epoch_with_minimum_testing_loss = 0
    saved_cells_that_were_wrong_list = []
    saved_wrong_fibroblasts = 0
    saved_wrong_cancer_cells = 0

    # Saved testing_data testing
    if superloop_epoch_with_maximum_accuracy + 1 < 10:
        model.load_weights(os.path.join(shortened_weight_save_folder, r"Superloop_run_{0}".format(superloop_counter_maximum_accuracy), epoch_name_part_1 + "0" + str(superloop_epoch_with_maximum_accuracy + 1) + epoch_name_part_2))
    else:
        model.load_weights(os.path.join(shortened_weight_save_folder, r"Superloop_run_{0}".format(superloop_counter_maximum_accuracy), epoch_name_part_1 + str(superloop_epoch_with_maximum_accuracy + 1) + epoch_name_part_2))
    correct_counter = 0
    # Testing examples * 4 due to the rotated matrices...

    for i in range(number_testing_examples*16):
        ...
        if current_prediction_index == testing_answer_index:
            ...
            saved_cells_that_were_wrong_list.append(final_saved_testing_data[i])
            if testing_answer[0] == 0:
                # It's a cancer cell!
                saved_wrong_cancer_cells = saved_wrong_cancer_cells + 1
            else:
                # It's a fibroblast!
                saved_wrong_fibroblasts = saved_wrong_fibroblasts + 1

        #print(model.predict(current_example), testing_answer, is_correct, correct_counter, final_testing_data[i])


    test_acc = model.evaluate_generator(generator = saved_test_gen, verbose=1)
    print('\nTest loss:', test_acc)
    if(correct_counter > saved_maximum_accuracy):
        saved_maximum_accuracy = correct_counter
        saved_testing_loss_with_maximum_accuracy = test_acc
        saved_epoch_with_maximum_accuracy = saved_superloop_epoch_with_maximum_accuracy + 1
    if(saved_minimum_testing_loss > test_acc):
        saved_minimum_testing_loss = test_acc
        saved_accuracy_with_minimum_testing_loss = correct_counter
        saved_epoch_with_minimum_testing_loss = superloop_epoch_with_maximum_accuracy + 1
    print("Total number of wrong cancer cells = " + str(saved_wrong_cancer_cells))
    print("Total number of wrong fibroblasts = " + str(saved_wrong_fibroblasts))
    print("Cells that were wrong: " + str(saved_cells_that_were_wrong_list))
    saved_cells_that_were_wrong_list = []
    saved_wrong_fibroblasts = 0
    saved_wrong_cancer_cells = 0
    is_correct = False
    model.summary()
    print("For saved testing cells, the maximum accuracy was " + str(saved_maximum_accuracy)
          + " with testing loss " + str(saved_testing_loss_with_maximum_accuracy)
           + " at epoch " + str(saved_epoch_with_maximum_accuracy))
    print("For saved testing cells, the minimum testing loss was " + str(saved_minimum_testing_loss)
          + " with accuracy " + str(saved_accuracy_with_minimum_testing_loss)
           + " at epoch " + str(saved_epoch_with_minimum_testing_loss))
    print("Total number of epochs = "+ str(nb_epochs))
    print("For saved testing cells, Total number of wrong cancer cells = " + str(saved_wrong_cancer_cells))
    print("For saved testing cells, Tpta; number of wrong fibroblasts = " + str(saved_wrong_fibroblasts))
    #print("List of wrong cells: " + str(cells_that_were_wrong_list))
    if (saved_maximum_accuracy > saved_superloop_maximum_accuracy):
        saved_superloop_maximum_accuracy = saved_maximum_accuracy
        saved_superloop_testing_loss_with_maximum_accuracy = saved_testing_loss_with_maximum_accuracy
        saved_superloop_epoch_with_maximum_accuracy = saved_epoch_with_maximum_accuracy
        saved_superloop_counter_maximum_accuracy = superloop_counter
    if (saved_superloop_minimum_testing_loss > saved_minimum_testing_loss):
        saved_superloop_minimum_testing_loss = saved_minimum_testing_loss
        saved_superloop_accuracy_with_minimum_testing_loss = saved_accuracy_with_minimum_testing_loss
        saved_superloop_epoch_with_minimum_testing_loss = saved_epoch_with_minimum_testing_loss
        saved_superloop_counter_minimum_testing_loss = superloop_counter
    #print("Model running is +" + )
    print("For saved cells, The superloop maximum accuracy was " + str(saved_superloop_maximum_accuracy)
          + " with testing loss " + str(saved_superloop_testing_loss_with_maximum_accuracy)
          + " at epoch " + str(saved_superloop_epoch_with_maximum_accuracy)
          + " at superloop loop counter " + str(saved_superloop_counter_maximum_accuracy))
    print("For saved cells, The superloop minimum testing loss was " + str(saved_superloop_minimum_testing_loss)
          + " with accuracy " + str(saved_superloop_accuracy_with_minimum_testing_loss)
          + " at epoch " + str(saved_superloop_epoch_with_minimum_testing_loss)
          + " at superloop loop counter " + str(saved_superloop_counter_minimum_testing_loss))
    print("Total number of epochs across superloop = " + str(nb_epochs))
