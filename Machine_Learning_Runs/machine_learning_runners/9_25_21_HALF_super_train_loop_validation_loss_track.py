...
THINGS TO PROGRAM
RUN THE MACHINE LEARNING WHILE INTENTIONALLY CUTTING CELLS FROM THE TRAINING DATASET.
This will involve 
'''
...
def get_the_rotated_files():
    # MODIFY THIS SO THAT TRAINING IS NOT JUST AN ELSE. Need a limit on the number of training cells
    # Total training images = 8352
    training_data_dict = {}
    ...
    # Remember that the actual number of cells for each time is actually 4x what's displayed...
    # Originally, the cells would sum to 2088, which is 25% of 8352, the total number of cells avaliable for training
    # Now we want the cells to sum to 1/2 of that, 1044.
    ...
    print("testing_saving_cancer_counter = " + str(testing_saving_cancer_counter))
    print("testing_saving_counter = " + str(testing_saving_counter))
    assert len(training_labels) == 1044
    assert len(training_data_dict["training_data_1"]) == 1044
    assert len(training_data_dict["training_data_2"]) == 1044
    assert len(training_data_dict["training_data_3"]) == 1044
    assert len(training_data_dict["training_data_4"]) == 1044
    '''
    ...
    assert len(shuffled_training_data) == 4176
    assert len(shuffled_training_labels) == 4176
...
for superloop_counter in range(superloop_rounds):
    ...
    weight_save_folder=r"D:\MIT_Tumor_Identifcation_Project_Stuff\June_Finishing_Project\Runs\Half_Training_All_Images\11_19_21_Test18_(half_training)\Superloop_run_{0}".format(superloop_counter)
    ...
    filepath=os.path.join(weight_save_folder,"11_19_21_Test18_DNN.{epoch:02d}.hdf5")
    filename=os.path.join(weight_save_folder,'training.log')
    text_results_file_name = "11_19_21_test18_half_training_set.txt"
    model_name_to_use = "new_20X_model_4"
    step_decay_thing = "step_decay_modify_2"
    checkDirectory(weight_save_folder)

    epoch_name_part_1 = r"11_19_21_Test18_DNN."
...
'''
directory_to_write_to = r"D:\MIT_Tumor_Identifcation_Project_Stuff\June_Finishing_Project\Runs\Half_Training_All_Images"

checkDirectory(directory_to_write_to)

file1 = open(os.path.join(directory_to_write_to,text_results_file_name),"a")#append mode
file1.write("11_19_21_test_results_18  \n")
