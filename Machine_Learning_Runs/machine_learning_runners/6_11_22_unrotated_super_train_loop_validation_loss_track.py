...
def get_the_unrotated_files():
    #TODO: Ensure that the testing cells are 400 in number.
    training_data_dict = {}
    ...
    assert len(shuffled_training_data) == 2088
    assert len(shuffled_validation_data) == 100
...
for superloop_counter in range(superloop_rounds):
    ...
    weight_save_folder=r"D:\MIT_Tumor_Identifcation_Project_Stuff\June_Finishing_Project\Runs\Cut_rotation_all_runs\All_Training_Images\6_11_22_cut_rotate_run_25\Superloop_run_{0}".format(superloop_counter)
    ...
    filepath=os.path.join(weight_save_folder,"6_11_22_Test25_DNN.{epoch:02d}.hdf5")
    filename=os.path.join(weight_save_folder,'training.log')
    text_results_file_name = "6_11_22_test25_set.txt"
    model_name_to_use = "new_20X_model_4"
    step_decay_thing = "step_decay_modify_2"
    checkDirectory(weight_save_folder)

    epoch_name_part_1 = r"6_11_22_Test25_DNN."
...
directory_to_write_to = r"D:\MIT_Tumor_Identifcation_Project_Stuff\June_Finishing_Project\Runs\Cut_rotation_all_runs\All_Training_Images"

checkDirectory(directory_to_write_to)

file1 = open(os.path.join(directory_to_write_to,text_results_file_name),"a")#append mode
file1.write("6_11_22_test_results_25  \n")
