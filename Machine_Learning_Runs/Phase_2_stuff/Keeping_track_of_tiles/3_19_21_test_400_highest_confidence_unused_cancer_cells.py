...
directory_of_cells = r"D:\MIT_Tumor_Identifcation_Project_Stuff\Phase_2_stuff\New_20X_images\Macro V5 images\20X tile_watch\unused_cancer_cells"
...
# using naive methods
list_of_cancer_cell_confidences = []
counter = 0
for path in final_compiled_testing_data:
    if "accuracy" in path:
        print(path)
        for ii in range(len(path)):
            if path[ii] == "F":
                if path[ii + 1] == "b":
                    letters_before_fb = path[:ii]
                    letters_after_fb = path[ii:]
                    ...
                    print("cancer cell number = " + str(cancer_cell_number))
                    list_of_cancer_cell_confidences.append(cancer_cell_number)
#list_of_cancer_cell_confidences.reverse()
for x in range(len(final_compiled_testing_data)):
    print(final_compiled_testing_data[x])
    print(final_compiled_testing_labels[x])
    print(list_of_cancer_cell_confidences[x])

for x in range(len(list_of_cancer_cell_confidences) - 400):
    current_minimum = min(list_of_cancer_cell_confidences)
    for i in range(len(final_compiled_testing_data)):
        if list_of_cancer_cell_confidences[i] == current_minimum:
            print(list_of_cancer_cell_confidences[i])
            # A minimum confidence has been found. Pop the values there...
            final_compiled_testing_labels.pop(i)
            final_compiled_testing_data.pop(i)
            list_of_cancer_cell_confidences.pop(i)
            #cancer_cell_counter = cancer_cell_counter - 1
            break
print(len(final_compiled_testing_labels))
print(len(final_compiled_testing_data))
print(len(list_of_cancer_cell_confidences))
...
for i in range(len(final_compiled_testing_data)):
    ...
    if current_prediction_index == testing_answer_index:
        ...
        incorrect_cancer_cells_list.append(final_compiled_testing_data[i])
...
print("The maximum accuracy was " + str(correct_counter)
      + " with testing loss " + str(test_acc)
      + " out of " + str(len(final_compiled_testing_data)))
print("Total number of epochs = " + str(nb_epochs))
for x in incorrect_cancer_cells_list:
    print(x)
print(len(incorrect_cancer_cells_list))
