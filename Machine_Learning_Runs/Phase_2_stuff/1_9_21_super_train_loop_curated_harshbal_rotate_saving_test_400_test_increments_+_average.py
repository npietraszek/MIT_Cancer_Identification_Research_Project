...
def get_the_rotated_files():
    ...
    number_testing_cancer_cells = 250
    number_testing_fibroblasts = 250
...
average_superloop_maximum_accuracy = 0
average_superloop_testing_loss_with_maximum_accuracy = 0
average_superloop_epoch_with_maximum_accuracy = 0
average_superloop_counter_maximum_accuracy = 0

average_superloop_minimum_testing_loss = 100000
average_superloop_accuracy_with_minimum_testing_loss = 0
average_superloop_epoch_with_minimum_testing_loss = 0
average_superloop_counter_minimum_testing_loss = 0
...
for superloop_counter in range(superloop_rounds):
    ...
    weight_save_folder=r"D:\MIT_Tumor_Identifcation_Project_Stuff\Phase_2_stuff\Machine Learning\Weights\1_9_21_new_run_3_(new_model_4,no_standard-harsh_bal(20-50-50),0.001_learning,5_400_cell_tests)\Superloop_run_{0}".format(superloop_counter)
    ...
    filepath=os.path.join(weight_save_folder,"1_9_21_Test3_balanced_0.001learning_DNN.{epoch:02d}.hdf5")
    filename=os.path.join(weight_save_folder,'training.log')
    checkDirectory(weight_save_folder)

    epoch_name_part_1 = r"1_9_21_Test3_balanced_0.001learning_DNN."
    ...
    testing_generator_dictionary = {}

    for test_generator_number in range(5):
        test_generator_index_begin = 400 * test_generator_number
        test_generator_index_end = 400 * (test_generator_number + 1)
        testing_data_subsection = final_testing_data[test_generator_index_begin:test_generator_index_end]
        testing_label_subsection = final_testing_labels[test_generator_index_begin:test_generator_index_end]
        test_gen = nicholas_generator(testing_data_subsection,testing_label_subsection,batch_sz)
        testing_generator_dictionary["generator_{0}".format(test_generator_number)] = test_gen
    config.gpu_options.allow_growth = True
    ...
    for epoch_counter in range(nb_epochs):
        ...
        correct_counter = 0
        for test_set_number in range(5):
            correct_counter = 0
            # Testing examples * 4 due to the rotated matrices...
            for i in range(number_testing_examples*4):
                current_example = np.expand_dims((np.array(np.load(final_testing_data[i+400*test_set_number]))),axis = 0)
                current_prediction = model.predict(current_example)
                testing_answer = np.array(np.load(final_testing_labels[i+400*test_set_number]))
                ...
                if current_prediction_index == testing_answer_index:
                    ...
                    cells_that_were_wrong_list.append(final_testing_data[i+400*test_set_number])
            ...
            test_acc = model.evaluate_generator(generator = testing_generator_dictionary["generator_{0}".format(test_set_number)], verbose=1)


            # Average appending
            print('\nTest loss:', test_acc)
            average_superloop_maximum_accuracy = average_superloop_maximum_accuracy + correct_counter
            average_superloop_testing_loss_with_maximum_accuracy = average_superloop_testing_loss_with_maximum_accuracy + test_acc

            #average_superloop_minimum_testing_loss = average_superloop_minimum_testing_loss + correct_counter
            #average_superloop_accuracy_with_minimum_testing_loss = average_superloop_accuracy_with_minimum_testing_loss + test_acc
        ...
        average_superloop_maximum_accuracy = average_superloop_maximum_accuracy / 5
        average_superloop_testing_loss_with_maximum_accuracy = average_superloop_testing_loss_with_maximum_accuracy / 5

        model.summary()
        print("The maximum accuracy was " + str(average_superloop_maximum_accuracy)
              + " with testing loss " + str(average_superloop_testing_loss_with_maximum_accuracy)
              + " at epoch " + str(epoch_counter))
        print("Total number of epochs = " + str(nb_epochs))
        # print("List of wrong cells: " + str(cells_that_were_wrong_list))
        if (average_superloop_maximum_accuracy > superloop_maximum_accuracy):
            superloop_maximum_accuracy = average_superloop_maximum_accuracy
            superloop_testing_loss_with_maximum_accuracy = average_superloop_testing_loss_with_maximum_accuracy
            superloop_epoch_with_maximum_accuracy = epoch_counter
            superloop_counter_maximum_accuracy = superloop_counter
        if (superloop_minimum_testing_loss > average_superloop_testing_loss_with_maximum_accuracy):
            superloop_minimum_testing_loss = average_superloop_testing_loss_with_maximum_accuracy
            superloop_accuracy_with_minimum_testing_loss = average_superloop_maximum_accuracy
            superloop_epoch_with_minimum_testing_loss = epoch_counter
