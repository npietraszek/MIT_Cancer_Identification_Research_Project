...
def test_all_training_cells_on_model_directory(directory):
    #import os
    ...
    # initializing list
    indices_list = []
    print("The original list has " + str(len(final_training_data)) + " entries.")

    no_duplicates_compiled_testing_data = []
    no_duplicates_compiled_testing_labels = []
    for i in range(len(final_training_data)):
        if os.path.split(final_training_data[i])[0] not in no_duplicates_compiled_testing_data:
            no_duplicates_compiled_testing_data.append(os.path.split(final_training_data[i])[0])
            no_duplicates_compiled_testing_labels.append(os.path.split(final_training_labels[i])[0])
        else:
...
header = ["Model title", 'Training cell set accuracy', 'Training cell accuracy percentage', 'Minimum validation loss epoch number',
...
for directory in next(os.walk(directory_to_use))[1]:
    ...
    data = test_all_training_cells_on_model_directory(true_directory)
    total_data.append(data)

header_present = False

if os.path.isfile(
        os.path.join(directory_to_use,r"Training_cells_results.csv")):
    with open(
            os.path.join(directory_to_use,r"Training_cells_results.csv"),
            'r', newline='', encoding='UTF8') as f:
        reader = csv.reader(f)
        for row in reader:
            if row == ["Model title", 'Training cell set accuracy', 'Training cell accuracy percentage', 'Minimum validation loss epoch number',
              'Correct Cancer', "Correct Fibroblasts", "Wrong Cancer", "Wrong Fibroblast"]:
                header_present = True
                break
    f.close()
...
with open(os.path.join(directory_to_use,r"Training_cells_results.csv"),
