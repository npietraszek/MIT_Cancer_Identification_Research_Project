'''
5/29/21
This CSV file creator is designed to look at all the ROIs that are part of the 1600 cell network
and create CSV files based on the machine learning's predictions based on every single one.
Unlike 5_10_21_CSV_File_creator_Macro_V5.py, which takes into account that some cells were curated out
(either because they were excess unused cancer cells or because the ground truth didn't match the confidence
for identifying what cell type each cell is)
this file takes ALL cells in the ROI, including ones not used in the machine learning testing itself.

As of 5/29/21 there are 760 unique cells identified this way for the current 1600 cells configuration
much more than the 408 unique cells in the 1600 cells after curation.

Some cells were still curated out, because they were too big for the image standard of 20x50x50.
There is nothing we can do about that.
However, the number of cells curated out this way is much, much smaller, and many of the ROI sections
do indeed have all their CSVs in order.
...
step_5_directory = r"D:\MIT_Tumor_Identifcation_Project_Stuff\Phase_2_stuff\New_20X_images\Macro V5 images\20X tile_watch\step5 4D matrices"
...
# No longer needed... we do not need the actual 1600 cells. We only need the directories from there.
'''
# Must read datasets from text file
file1 = open(os.path.join(r"D:\MIT_Tumor_Identifcation_Project_Stuff\Phase_2_stuff\Machine Learning\Weights\3_25_21_new_run_1_(new_model_4,no_standard-harsh_bal(20-50-50),tile_keep_1600_seperate)\Superloop_run_4","Datasets.txt"),"r")
...
# Need to be able to get cell folders

old_directory_prefix_list = next(os.walk(saved_1600_cells_directory))[1]
print(old_directory_prefix_list)

directory_prefix_list = []

# (TO DO) Need to adjust these prefixes so that they can properly read it
for the_directory in old_directory_prefix_list:
    directory_prefix_list.append(the_directory[0:-4])
for the_directory in directory_prefix_list:
    print("New directory_prefix_list entry: " + the_directory)

step_5_total_directory_list = []

for root, dirs, files in os.walk(step_5_directory):
    for dir in dirs:
        selected_directory_string = str(dir)
        # print(selected_directory_string)

        for prefix_counter in range(len(directory_prefix_list)):
            if directory_prefix_list[prefix_counter] in selected_directory_string:
                step_5_total_directory_list.append(selected_directory_string)
                print("Matched " + directory_prefix_list[prefix_counter] + " inside " + selected_directory_string)

print(len(step_5_total_directory_list))

final_compiled_testing_data = []
final_compiled_testing_labels = []

# Need to join path with step 5 directory

for path_index in range(len(step_5_total_directory_list)):
    final_compiled_testing_data.append(os.path.join(step_5_directory,step_5_total_directory_list[path_index],"Final_5D_array.npy"))
    final_compiled_testing_labels.append(os.path.join(step_5_directory,step_5_total_directory_list[path_index],"Label_matrix.npy"))
...
# D:\MIT_Tumor_Identifcation_Project_Stuff\Phase_2_stuff\New_20X_images\Macro V5 images\20X tile_watch\step5 4D matrices\device 1 chip 1 and 2 ROI1_02.oib - Series 1-1 0000_cell0_0.2747Fb0.7253Tc_accuracy58.1774index6\Final_5D_array_1.npy
# D:\MIT_Tumor_Identifcation_Project_Stuff\Phase_2_stuff\New_20X_images\Macro V5 images\20X tile_watch\step5 4D matrices\device 1 chip 1 and 2 ROI1_02.oib - Series 1-1 0000_cell0_0.2747Fb0.7253Tc_accuracy58.1774index6
...
# using naive methods
print("Length of device_directory_list is " + str(len(device_directory_list)))
print("Length of final_testing_directories_list is " + str(len(final_testing_directories_list)))
print("Length of final_compiled_testing_data is " + str(len(final_compiled_testing_data)))
...
model.load_weights(os.path.join(r"D:\MIT_Tumor_Identifcation_Project_Stuff\June_Finishing_Project\Runs\Extra_runs_with_best_model\11_15_21_Test6\Superloop_run_0",r"11_15_21_Test6_DNN.14.hdf5"))
correct_counter = 0
...
# D:\MIT_Tumor_Identifcation_Project_Stuff\Phase_2_stuff\New_20X_images\Macro V5 images\20X tile_watch\step5 4D matrices\device 1 chip 1 and 2 ROI1_02.oib - Series 1-1 0000_cell0_0.2747Fb0.7253Tc_accuracy58.1774index6\Final_5D_array.npy
# D:\MIT_Tumor_Identifcation_Project_Stuff\May_Reconstruction\new 20x batch Testing with Tuan's V5 Macro\device 1\chip 1 and 2\ROI1_02\_tiles\Result\ROI1_02.oib - Series 1-1 0000\ROI1_02.oib - Series 1-1 0000cell0
# Correct!
model.summary()
print("The maximum accuracy was " + str(correct_counter))
print("Total accuracy percentage is " + str((correct_counter / len(final_compiled_testing_data)) * 100) + "%.")
