...
for superloop_counter in range(superloop_rounds):
    ...
    weight_save_folder=r"D:\MIT_Tumor_Identifcation_Project_Stuff\Phase_2_stuff\Machine Learning\Weights\5_12_21_Test1_(new_20X_model_4,step_decay_modify_2)\Superloop_run_{0}".format(superloop_counter)
    ...
    filepath=os.path.join(weight_save_folder,"5_12_21_Test1_DNN.{epoch:02d}.hdf5")
    filename=os.path.join(weight_save_folder,'training.log')
    text_results_file_name = "5_12_21_test1_results_rotated_1600_set.txt"
    model_name_to_use = "new_20X_model_4"
    step_decay_thing = "step_decay_modify_2"
    checkDirectory(weight_save_folder)

    epoch_name_part_1 = r"5_12_21_Test1_DNN."
    ...
    # NEEDS TESTING
    if epoch_with_minimum_validation_loss + 1 < 10:
        model.load_weights(
            os.path.join(shortened_weight_save_folder, r"Superloop_run_{0}".format(str(epoch_with_minimum_validation_loss)), epoch_name_part_1 + "0" + str(epoch_with_minimum_validation_loss) + epoch_name_part_2))
    else:
        model.load_weights(
            os.path.join(shortened_weight_save_folder, r"Superloop_run_{0}".format(str(epoch_with_minimum_validation_loss)), epoch_name_part_1 + str(epoch_with_minimum_validation_loss) + epoch_name_part_2))
...
file1.write("5_16_21_test_results_1 \n")
