...
def get_the_unrotated_files():
    ...
    # They were originally 50 each, we now need to make them 5 each to reduce to 10%
    number_validation_cancer_cells = 5
    number_validation_fibroblasts = 5
    number_testing_cancer_cells = 5
    number_testing_fibroblasts = 5
    # Original number of cells is 8352.
    # With the rotations taken off, we now have 1/4 of that, which is 2088
    # We now want to further constrain to 10% of that new amount, which is 209
    # This is 105 cancer cells and 105 fibroblasts.
    number_training_cancer_cells = 105
    number_training_fibroblasts = 105
    ...
    assert len(shuffled_training_data) == 210
    assert len(shuffled_validation_data) == 10
    assert len(shuffled_testing_data) == 10
...
for superloop_counter in range(superloop_rounds):
    ...
    weight_save_folder=r"D:\MIT_Tumor_Identifcation_Project_Stuff\June_Finishing_Project\Runs\Cut_rotation_all_runs\10%_training_images\12_8_21_cut_rotate_run_25\Superloop_run_{0}".format(superloop_counter)
    ...
    filepath=os.path.join(weight_save_folder,"12_8_21_Test25_DNN.{epoch:02d}.hdf5")
    filename=os.path.join(weight_save_folder,'training.log')
    text_results_file_name = "12_8_21_test25_set.txt"
    model_name_to_use = "new_20X_model_4"
    step_decay_thing = "step_decay_modify_2"
    checkDirectory(weight_save_folder)

    epoch_name_part_1 = r"12_8_21_Test25_DNN."
...
directory_to_write_to = r"D:\MIT_Tumor_Identifcation_Project_Stuff\June_Finishing_Project\Runs\Cut_rotation_all_runs\10%_training_images"

checkDirectory(directory_to_write_to)

file1 = open(os.path.join(directory_to_write_to,text_results_file_name),"a")#append mode
file1.write("12_8_21_test_results_25  \n")
