...
# Total accuracy and loss values for confidence interval
superloop_confidence_interval_maximum_accuracy = 0
superloop_confidence_interval_maximum_accuracy_epoch = 0
superloop_confidence_interval_maximum_accuracy_superloop_counter = 0
...
for superloop_counter in range(superloop_rounds):
    ...
    weight_save_folder=r"D:\MIT_Tumor_Identifcation_Project_Stuff\Phase_2_stuff\Machine Learning\Weights\4_16_21_new_run_3_(new_20X_model_12,step_decay_modify_2)\Superloop_run_{0}".format(superloop_counter)
    ...
    filepath=os.path.join(weight_save_folder,"4_16_21_Test3_DNN.{epoch:02d}.hdf5")
    filename=os.path.join(weight_save_folder,'training.log')
    text_results_file_name = "4_16_21_test3_results_rotated_1600_set.txt"
    model_name_to_use = "new_20X_model_12"
    step_decay_thing = "step_decay_modify_2"
    checkDirectory(weight_save_folder)

    epoch_name_part_1 = r"4_16_21_Test3_DNN."
    ...
    print(len(final_validation_data))
    ...
    #
    # # training_maximum_accuracy = 0
    # # validation_loss_with_maximum_accuracy = 0
    # # epoch_with_validation_maximum_accuracy = 0
    ...
    #
    #         current_example = np.expand_dims((np.array(np.load(final_training_data[i]))), axis=0)
    #         current_prediction = model.predict(current_example)
    #         testing_answer = np.array(np.load(final_training_labels[i]))
    ...
    #             is_correct = True
    #         else:
    #             cells_that_were_wrong_list.append(final_training_data[i])
    ...
    #     print("Training accuracy: " + str(correct_counter) + " out of " + str(len(final_training_data)))
    ...
    for epoch_counter in range(nb_epochs):
        ...
        if epoch_counter + 1 < 10:
            model.load_weights(
                os.path.join(weight_save_folder,
                             epoch_name_part_1 + "0" + str(epoch_counter + 1) + epoch_name_part_2))
        else:
            model.load_weights(
                os.path.join(weight_save_folder, epoch_name_part_1 + str(epoch_counter + 1) + epoch_name_part_2))
        correct_counter = 0
        # Testing examples * 4 due to the rotated matrices...
        for i in range(len(final_validation_data)):

            current_example = np.expand_dims((np.array(np.load(final_validation_data[i]))), axis=0)
            current_prediction = model.predict(current_example)
            testing_answer = np.array(np.load(final_validation_labels[i]))
            current_prediction_index = 0
            testing_answer_index = 0
            for x in range(2):
                if current_prediction[0][x] == max(current_prediction[0]):
            ...
            if current_prediction_index == testing_answer_index:
                ...
                cells_that_were_wrong_list.append(final_validation_data[i])
                if testing_answer[0] == 0:
                    # It's a cancer cell!
                    wrong_cancer_cells = wrong_cancer_cells + 1
                else:
            ...
            # print(model.predict(current_example), testing_answer, is_correct, correct_counter, final_testing_data[i])
        print("Current epoch is " + str(epoch_counter))
        print("Validation accuracy: " + str(correct_counter) + " out of " + str(len(final_validation_data)))
        # test_acc = model.evaluate_generator(generator = test_gen, verbose=1)
        # print('\nTest loss:', test_acc)
    ...
    wrong_fibroblasts = 0
    wrong_cancer_cells = 0
    # 95% confidence interval trackers for this superloop only
    confidence_interval_maximum_accuracy = 0
    confidence_interval_maximum_accuracy_epoch = 0
    ...
    for epoch_counter in range(nb_epochs):
        ...
        #print("Final testing data's length is " + str(len(final_testing_data)))
        #print("Number of testing examples * 4 is " + str(number_testing_examples * 4))
        ...
        print("Testing accuracy: " + str(correct_counter) + " out of " + str(len(final_testing_data)))
        # Comment out confidence interval as it takes too long and doesn't really tell us much helpful


        # Test the saved_datasets to do the 95% confidence interval
        ...
        for x in range(30):
            ...
            if epoch_counter + 1 < 10:
                the_path = os.path.join(weight_save_folder,
                                 epoch_name_part_1 + "0" + str(epoch_counter + 1) + epoch_name_part_2)
                model.load_weights(the_path)
                # D:\MIT_Tumor_Identifcation_Project_Stuff\Phase_2_stuff\Machine Learning\Weights\4_6_21_new_run_1_(new_20X_model_12,step_decay_modify_2)\Superloop_run_0\4_6_21_Test1_DNN.01.hdf5
            else:
                the_path = os.path.join(weight_save_folder, epoch_name_part_1 + str(epoch_counter + 1) + epoch_name_part_2)
                model.load_weights(the_path)

            confidence_interval_correct_counter = 0
            # Testing examples * 4 due to the rotated matrices...
            for i in range(400):
                is_correct = False
                ...
                for x in range(2):
                    ...
                    if testing_answer[x] == max(testing_answer):
                        testing_answer_index = x
                if current_prediction_index == testing_answer_index:
                    confidence_interval_correct_counter += 1
                    is_correct = True

                # print(model.predict(current_example), testing_answer, is_correct, confidence_interval_correct_counter, compiled_testing_data[i])
            #test_acc = model.evaluate_generator(generator=test_gen, verbose=0)
            #print('\nTest loss:', test_acc)

            #model.summary()
            #print("The maximum accuracy was " + str(confidence_interval_correct_counter)
            #      + " with testing loss " + str(test_acc))
            #print("Total number of epochs = " + str(nb_epochs))
            list_of_accuracy_values.append(confidence_interval_correct_counter)
        
        mean_of_accuracy_values = np.mean(list_of_accuracy_values)
        print("The mean confidence interval value was " + str(mean_of_accuracy_values) + " out of 400")
        
        # Confidence interval stuff
        if(mean_of_accuracy_values > confidence_interval_maximum_accuracy):
            confidence_interval_maximum_accuracy = mean_of_accuracy_values
            confidence_interval_maximum_accuracy_epoch = epoch_counter + 1

        # TRIPLE QUOTES WOULD GO HERE
        #test_acc = model.evaluate_generator(generator = test_gen, verbose=1)
        #print('\nTest loss:', test_acc)


        # After all the testing has been accomplished, compare the best scores for both accuracy and loss
        ...
        is_correct = False
    model.summary()
    print("The confidence interval maximum accuracy is " + str(confidence_interval_maximum_accuracy) +
          " at epoch " + str(confidence_interval_maximum_accuracy_epoch))
    print("The maximum accuracy was " + str(maximum_accuracy)
          + " with validation loss " + str(validation_loss_with_maximum_accuracy)
           + " at epoch " + str(epoch_with_maximum_accuracy))
    print("The minimum validation loss was " + str(minimum_validation_loss)
    ...
    # Confidence interval total accuracy checking
    if (confidence_interval_maximum_accuracy > superloop_confidence_interval_maximum_accuracy):
        superloop_confidence_interval_maximum_accuracy = confidence_interval_maximum_accuracy
        superloop_confidence_interval_maximum_accuracy_epoch = confidence_interval_maximum_accuracy_epoch
        superloop_confidence_interval_maximum_accuracy_superloop_counter = superloop_counter
    print("                 ----CONFIDENCE INTERVAL MAXIMUM ACCURACY----                ")
    print("The superloop confidence interval maximum accuracy was " + str(superloop_maximum_accuracy)
          + " at epoch " + str(superloop_confidence_interval_maximum_accuracy_epoch)
          + " at superloop loop counter " + str(superloop_confidence_interval_maximum_accuracy_superloop_counter))
...
file1 = open(os.path.join(shortened_weight_save_folder, r"Superloop_run_{0}".format(str(superloop_counter_minimum_validation_loss)), r"Datasets.txt"), "r")
datasets = file1.readlines()
...
for x in range(100):
    ...
    if superloop_epoch_with_maximum_validation_accuracy + 1 < 10:
        model.load_weights(
...
file1.write("4_16_21_test_results_3 \n")
