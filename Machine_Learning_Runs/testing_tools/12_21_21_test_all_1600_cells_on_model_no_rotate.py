...
# MUST WRITE THIS TO TEST ONLY THE 1600 CELLS THAT AREN'T ROTATED...
...
def test_all_1600_cells_on_model_directory(directory):
    #import os
...
directory_to_use = r"D:\MIT_Tumor_Identifcation_Project_Stuff\June_Finishing_Project\Runs\Cut_rotation_all_runs\10%_Training_Images"

header = ["Model title", '1600 cell set accuracy', '1600 cell accuracy percentage', 'Minimum validation loss epoch number',
...
for directory in next(os.walk(directory_to_use))[1]:
    ...
    data = test_all_1600_cells_on_model_directory(true_directory)
    total_data.append(data)

header_present = False

if os.path.isfile(
        os.path.join(directory_to_use,r"results.csv")):
    with open(
            os.path.join(directory_to_use,r"results.csv"),
            'r', newline='', encoding='UTF8') as f:
        reader = csv.reader(f)
        for row in reader:
            if row == ["Model title", '1600 cell set accuracy', '1600 cell accuracy percentage', 'Minimum validation loss epoch number',
              'Correct Cancer', "Correct Fibroblasts", "Wrong Cancer", "Wrong Fibroblast"]:
                header_present = True
                break
    f.close()
...
with open(os.path.join(directory_to_use,r"results.csv"),
