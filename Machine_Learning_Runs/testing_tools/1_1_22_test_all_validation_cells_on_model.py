...
def test_all_validation_cells_on_model_directory(directory):
    #import os
    ...
    # initializing list
    indices_list = []
    print("The original list has " + str(len(final_validation_data)) + " entries.")

    no_duplicates_compiled_testing_data = []
    no_duplicates_compiled_testing_labels = []
    for i in range(len(final_validation_data)):
        if os.path.split(final_validation_data[i])[0] not in no_duplicates_compiled_testing_data:
            no_duplicates_compiled_testing_data.append(os.path.split(final_validation_data[i])[0])
            no_duplicates_compiled_testing_labels.append(os.path.split(final_validation_labels[i])[0])
        else:
    ...
    # MUST FIX THIS
    # maybe just find the first file inside this directory and use it as a template
...
directory_to_use = r"D:\MIT_Tumor_Identifcation_Project_Stuff\June_Finishing_Project\Runs\Extra_runs_with_best_model"

header = ["Model title", 'Validation cell set accuracy', 'Validation cell accuracy percentage', 'Minimum validation loss epoch number',
...
for directory in next(os.walk(directory_to_use))[1]:
    ...
    data = test_all_validation_cells_on_model_directory(true_directory)
    total_data.append(data)

header_present = False

if os.path.isfile(
        os.path.join(directory_to_use,r"Validation_cells_results.csv")):
    with open(
            os.path.join(directory_to_use,r"Validation_cells_results.csv"),
            'r', newline='', encoding='UTF8') as f:
        reader = csv.reader(f)
        for row in reader:
            if row == ["Model title", 'Validation cell set accuracy', 'Validation cell accuracy percentage', 'Minimum validation loss epoch number',
              'Correct Cancer', "Correct Fibroblasts", "Wrong Cancer", "Wrong Fibroblast"]:
                header_present = True
                break
    f.close()
...
with open(os.path.join(directory_to_use,r"Validation_cells_results.csv"),
