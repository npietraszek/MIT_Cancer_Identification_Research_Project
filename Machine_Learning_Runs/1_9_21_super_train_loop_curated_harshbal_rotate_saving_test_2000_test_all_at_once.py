...
class kwabena_generator(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        return np.array([np.reshape(np.load(file_name)['input'], (64, 64, 64, 3)) for file_name in batch]), np.array(
            [np.reshape(np.load(file_name)['output'] / np.sum(np.load(file_name)['output'], 1), (3, 64, 64, 64)) for
             file_name in batch])
...
def get_the_rotated_files():
    ...
    shuffled_validation_data, shuffled_validation_labels = shuffle_matrices_list(validation_labels,
                                                                                 validation_data_dict)
...
for superloop_counter in range(superloop_rounds):
    ...
    weight_save_folder = r"D:\MIT_Tumor_Identifcation_Project_Stuff\Phase_2_stuff\Machine Learning\Weights\1_9_21_new_run_2_(new_model_4,no_standard-harsh_bal(20-50-50),0.001_learning,5_400_cell_tests)\Superloop_run_{0}".format(
        superloop_counter)
    ...
    config.gpu_options.allow_growth = True
    model.fit_generator(generator=train_gen, epochs=nb_epochs, validation_data=valid_gen, callbacks=callbacks_list,
                        shuffle="batch", initial_epoch=int_eph, verbose=2)
    ...
    for test_set_number in range(5):
        ...
        for epoch_counter in range(nb_epochs):
            ...
            if epoch_counter + 1 < 10:
                model.load_weights(os.path.join(weight_save_folder,
            ...
            for i in range(number_testing_examples * 4):
                ...
                current_example = np.expand_dims((np.array(np.load(final_testing_data[i + 400 * test_set_number]))),
                                                 axis=0)
            ...
            test_acc = model.evaluate_generator(
                generator=testing_generator_dictionary["generator_{0}".format(test_set_number)], verbose=1)
            print('\nTest loss:', test_acc)
        ...
        # Average appending
        average_superloop_maximum_accuracy = average_superloop_maximum_accuracy + superloop_maximum_accuracy
        average_superloop_testing_loss_with_maximum_accuracy = average_superloop_testing_loss_with_maximum_accuracy + superloop_testing_loss_with_maximum_accuracy

        average_superloop_minimum_testing_loss = average_superloop_minimum_testing_loss + superloop_minimum_testing_loss
        average_superloop_accuracy_with_minimum_testing_loss = average_superloop_accuracy_with_minimum_testing_loss + superloop_accuracy_with_minimum_testing_loss
