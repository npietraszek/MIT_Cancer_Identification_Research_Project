...
def get_the_rotated_files():
    ...
    # Now we want the cells to sum to 1/10 of that, 210.
    ...
    print("testing_saving_cancer_counter = " + str(testing_saving_cancer_counter))
    print("testing_saving_counter = " + str(testing_saving_counter))
    assert len(training_labels) == 210
    assert len(training_data_dict["training_data_1"]) == 210
    assert len(training_data_dict["training_data_2"]) == 210
    assert len(training_data_dict["training_data_3"]) == 210
    assert len(training_data_dict["training_data_4"]) == 210
    '''
    ...
    assert len(shuffled_training_data) == 840
    assert len(shuffled_training_labels) == 840
...
for superloop_counter in range(superloop_rounds):
    ...
    weight_save_folder=r"D:\MIT_Tumor_Identifcation_Project_Stuff\June_Finishing_Project\Runs\10%_Training_All_Images\11_21_21_Test16_(10%_training)\Superloop_run_{0}".format(superloop_counter)
    ...
    filepath=os.path.join(weight_save_folder,"11_21_21_Test16_DNN.{epoch:02d}.hdf5")
    filename=os.path.join(weight_save_folder,'training.log')
    text_results_file_name = "11_21_21_test16_10%_training_set.txt"
    model_name_to_use = "new_20X_model_4"
    step_decay_thing = "step_decay_modify_2"
    checkDirectory(weight_save_folder)

    epoch_name_part_1 = r"11_21_21_Test16_DNN."
...
'''
directory_to_write_to = r"D:\MIT_Tumor_Identifcation_Project_Stuff\June_Finishing_Project\Runs\10%_Training_All_Images"

checkDirectory(directory_to_write_to)

file1 = open(os.path.join(directory_to_write_to,text_results_file_name),"a")#append mode
file1.write("11_21_21_test_results_16  \n")
