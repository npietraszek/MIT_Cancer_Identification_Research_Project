...
def get_the_unrotated_files():
    #TODO: Fix the testing cells that there are 400 of them, and not 50.
    training_data_dict = {}
    ...
    # Need to take half of the cells for the validation and the testing as well
    # They were originally 50 each, we now need to make them 25 each to cut them by half
    # Ensuring that the testing cells are 400 to standardize the testing procedure
    number_validation_cancer_cells = 25
    number_validation_fibroblasts = 25
    number_testing_cancer_cells = 50
    number_testing_fibroblasts = 50
    # Original number of cells is 8352.
    # With the rotations taken off, we now have 1/4 of that, which is 2088
    # We now want to further constrain to 1/2 of that new amount, which is 1044.
    # This is 522 cancer cells and 522 fibroblasts.
    number_training_cancer_cells = 522
    number_training_fibroblasts = 522
    ...
    assert len(shuffled_training_data) == 1044
    assert len(shuffled_validation_data) == 50
    assert len(shuffled_testing_data) == 100
    assert len(shuffled_saved_testing_data) == 408
...
for superloop_counter in range(superloop_rounds):
    ...
    weight_save_folder=r"D:\MIT_Tumor_Identifcation_Project_Stuff\June_Finishing_Project\Runs\Cut_rotation_all_runs\50%_Training_Images_2\6_11_22_cut_rotate_run_19\Superloop_run_{0}".format(superloop_counter)
    ...
    saved_1600_cells_directory = r"D:\MIT_Tumor_Identifcation_Project_Stuff\May_Cutting_Image_Types\step9 Rotated_ROI_1600_cells"
    filepath=os.path.join(weight_save_folder,"6_11_22_Test19_DNN.{epoch:02d}.hdf5")
    filename=os.path.join(weight_save_folder,'training.log')
    text_results_file_name = "6_11_22_test19_set.txt"
    model_name_to_use = "new_20X_model_4"
    step_decay_thing = "step_decay_modify_2"
    checkDirectory(weight_save_folder)

    epoch_name_part_1 = r"6_11_22_Test19_DNN."
    ...
    final_testing_labels, final_saved_testing_data, final_saved_testing_labels = get_the_unrotated_files()
...
directory_to_write_to = r"D:\MIT_Tumor_Identifcation_Project_Stuff\June_Finishing_Project\Runs\Cut_rotation_all_runs\50%_Training_Images_2"

checkDirectory(directory_to_write_to)

file1 = open(os.path.join(directory_to_write_to,text_results_file_name),"a")#append mode
file1.write("6_11_22_test_results_19  \n")
