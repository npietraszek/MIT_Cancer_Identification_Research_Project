...
main_file_path = r"D:\MIT_Tumor_Identifcation_Project_Stuff\June_Finishing_Project\Runs\Half_Training_All_Images\11_19_21_Test2_(half_training)\Superloop_run_0"
...
model.load_weights(os.path.join(main_file_path,r"11_19_21_Test2_DNN."+ str(min_validation_epoch_number+1) + ".hdf5"))
correct_counter = 0
...
header = ['1600 cell set accuracy', '1600 cell accuracy percentage', 'Minimum validation loss epoch number', 'Correct Cancer', "Correct Fibroblasts", "Wrong Cancer", "Wrong Fibroblast"]
data = [str(correct_counter), str(100 * correct_counter / len(final_compiled_testing_data)) + "%",str(min_validation_epoch_number+1), str(cancer_correct), str(fibroblasts_correct), str(cancer_wrong), str(fibroblasts_wrong)]

# must first check if the header doesn't already exist in the csv file?

header_present = False

if os.path.isfile(r'D:\MIT_Tumor_Identifcation_Project_Stuff\June_Finishing_Project\Runs\Half_Training_All_Images\results.csv'):
    with open(r'D:\MIT_Tumor_Identifcation_Project_Stuff\June_Finishing_Project\Runs\Half_Training_All_Images\results.csv', 'r', newline='', encoding='UTF8') as f:
        reader = csv.reader(f)
        for row in reader:
            if row == ['1600 cell set accuracy', '1600 cell accuracy percentage', 'Minimum validation loss epoch number', 'Correct Cancer', "Correct Fibroblasts", "Wrong Cancer", "Wrong Fibroblast"]:
                header_present = True
                break
    f.close()
...
with open(r'D:\MIT_Tumor_Identifcation_Project_Stuff\June_Finishing_Project\Runs\Half_Training_All_Images\results.csv', 'a', newline='', encoding='UTF8') as f:
    writer = csv.writer(f)
